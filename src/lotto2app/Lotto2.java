...
/**
 *
 * @author Camelia Farcas, Conor Clarke
 */

public class Lotto2 implements Serializable{
   ...
   /* public Lotto2() {
        lottoNum = new int[5];
        for (int i = 0; i < 5; i++) {
            lottoNum[i] =(1+ (int) (Math.random() * 40));
        }

        plus1 = new int[5];
        for (int i = 0; i < 5; i++) {
            plus1[i] = (1+(int) (Math.random() * 40));
        }
        plus2 = new int[5];
        for (int i = 0; i < 5; i++) {
            plus2[i] =(1+ (int) (Math.random() * 40));
        }
        userGuess = new int[5];
        name = new String();
    }
